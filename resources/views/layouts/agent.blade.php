    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="p-4">
            <div class="sn-stat rounded-xl p-4">
                <p class="text-green-400/70 text-xs mb-1">Saldo Anda</p>
                <p class="text-2xl font-bold text-white">Rp {{ number_format(Auth::user()->balance ?? 0, 0, ',', '.') }}</p>
            </div>
        </div>

        <nav class="px-3 space-y-1">
            <a href="{{ route('agent.dashboard') }}" class="sn-nav-link flex items-center px-4 py-2.5 rounded-lg transition text-gray-300 {{ request()->routeIs('agent.dashboard') ? 'sn-nav-active' : '' }}">
                <i class="fas fa-home w-5 mr-3"></i><span>Dashboard</span>
            </a>
            <a href="{{ route('agent.vouchers.sell') }}" class="sn-nav-link flex items-center px-4 py-2.5 rounded-lg transition text-gray-300 {{ request()->routeIs('agent.vouchers*') ? 'sn-nav-active' : '' }}">
                <i class="fas fa-ticket w-5 mr-3"></i><span>Jual Voucher</span>
            </a>
            <a href="{{ route('agent.topup') }}" class="sn-nav-link flex items-center px-4 py-2.5 rounded-lg transition text-gray-300 {{ request()->routeIs('agent.topup*') ? 'sn-nav-active' : '' }}">
                <i class="fas fa-plus-circle w-5 mr-3"></i><span>Top Up Saldo</span>
            </a>
            <a href="{{ route('agent.transactions') }}" class="sn-nav-link flex items-center px-4 py-2.5 rounded-lg transition text-gray-300 {{ request()->routeIs('agent.transactions*') ? 'sn-nav-active' : '' }}">
                <i class="fas fa-history w-5 mr-3"></i><span>Riwayat Transaksi</span>
            </a>
            <a href="{{ route('agent.profile') }}" class="sn-nav-link flex items-center px-4 py-2.5 rounded-lg transition text-gray-300 {{ request()->routeIs('agent.profile*') ? 'sn-nav-active' : '' }}">
        ...
        <header class="sn-topbar sticky top-0 z-30">
            <div class="flex items-center justify-between px-5 py-3.5">
                ...
                <div class="flex items-center space-x-3 ml-auto">
                    <span class="text-gray-300 text-sm">{{ Auth::user()->name ?? 'Agent' }}</span>
